...
| 8 | 口袋中的發 | 本局主動使用，額外正式取得「發」 |
| 9 | 口袋中的中 | 本局主動使用，額外正式取得「中」 |
| 10 | 口袋中的白板 | 本局主動使用，額外正式取得「白板」 |

Phase 1-D 已完成三格 inventory、Commit 後取得、滿格強制替換、跨局保留與新場重置。嗆司Maker 每局第一次形成新聽牌時 +5 本局分數；Restart 會重新建立該局觸發狀態。

口袋系列可在摸牌階段主動使用，直接額外正式取得指定國字牌，不犧牲任何已取得牌。若指定牌已正式取得則不可使用。成功使用不增加摸牌張數，會將目標牌移出後續正式牌池與 Bonus 候選，並同步更新棋盤、已取得集合及連線／聽牌／牌型；同局可依序使用多個口袋系列 Item，每個成功使用後立即消失。Phase 1-E 已整合籤的動態權重與免洗護身符攔截。

## 10. 場中下注事件池

| # | 事件 | 成功條件 | 成功 | 失敗 |
|---:|---|---|---:|---:|
| 1 | 相信國聚 | 東、南、西、北、中、發、白 7 張中正式取得至少 6 張 | +30 | -30 |
